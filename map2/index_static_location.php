...
if(isset($_POST['ajax'])) {
	...
	if(isset($_POST['action']) && $_POST['action']=='get_nearby_stores') {
		...
		$sql = "SELECT *, ( 3959 * ACOS( COS( RADIANS(".$_POST['lat'].") ) * COS( RADIANS( latitude ) ) * COS( RADIANS( longitude ) - RADIANS(".$_POST['lng'].") ) + SIN( RADIANS(".$_POST['lat'].") ) * SIN( RADIANS( latitude ) ) ) ) AS distance FROM dealers WHERE status=1 AND approved=1 ".$category_filter." HAVING distance <= ".$_POST['distance']." ORDER BY distance ASC LIMIT 0,60";
...
if($_POST) {
	if(isset($_POST['address']) && empty($_POST['address'])) {
		...
		$tmp = file_get_contents("https://maps.google.com/maps/geo?q=".urlencode($_POST['address'])."&gl={$region}&output=xml&sensor=false&key={$google_api_key}");
		...
		if($xml['Response']['Status']['code']=='200') {
			...
			if(isset($coords[0]) && isset($coords[1])) {
				...
				$sql = "SELECT *, ( 3959 * ACOS( COS( RADIANS(".$coords[1].") ) * COS( RADIANS( latitude ) ) * COS( RADIANS( longitude ) - RADIANS(".$coords[0].") ) + SIN( RADIANS(".$coords[1].") ) * SIN( RADIANS( latitude ) ) ) ) AS distance FROM dealers WHERE status=1 HAVING distance <= ".$db->escape($_POST['distance'])." ORDER BY distance ASC  LIMIT 0,60";
...
<body id="super-store-finder">
	...
	<div id="wrapper">
		...
		<div class="links"></div>
			...
			<form method="post" action="./index.php" accept-charset="UTF-8" method="post" id="clinic-finder-form" class="clear-block" class="clear-block">
				...
				// support unicode
				mysql_query("SET NAMES utf8");
				$cats = $db->get_rows("SELECT categories.* FROM categories WHERE categories.id=2 ORDER BY categories.cat_name ASC");
  ...
  <div class="fb-like" data-href="https://superstorefinder.net/products/superstorefinder" data-send="false" data-layout="button_count" data-width="100" data-show-faces="false"></div>
