 ...
 */
 $(document).ready(function() {
	...
	$(document).on('click','.delete_trigger_click',function(){
		var data_url=$(this).attr('data-url');
		var data_id=$(this).attr('data-id');
		var update_to=$(this).attr('data-update-to');
				...
				if (Logout) {
					$.ajax({
						...
						success:function(data){
							...
							if(update_to == 'status'){
								if(value == 2){
									swalAl = "De-Activated Successfully..";
								}else{
									swalAl = "Activated Successfully..";
								}	
							}else{
								swalAl = "Deleted Successfully..";
							}
							swal(swalAl, {
								icon: "success",
							  }).then(() => {
...
$(document).on('submit','.pro_filter_form',function(e){
	...
	$(document).on('click','.custom_trigger_order_status',function(){
		var data_id=$(this).attr('data-id');
		$.ajax({
			url: base_url+'customizeorders/OrderStatusManagement',
			type: 'POST',
			dataType: 'json',
			data: {id: data_id},
			success:function(data){
	...
	$('.custom-update-order-status').click(function(e){
		e.preventDefault();
		var id=$('.order_id').val();
		var order_status=$('.order_status').val();
		var courier=$('.order_courier').val();
		...
		if(order_status==2){
	    	...
	    	$.ajax({
				url: base_url+'customizeorders/OrderStatusUpdate',
				type: 'POST',
				data: {id:id,status:order_status,courier:courier,tracking_code:tracking_id,notes:order_notes},
				success:function(data){
					$('#feature').modal('hide');
	...
	$(document).on('click','.custom_order_view_trigger',function(){
		var data_id=$(this).attr('data-id');
		$.ajax({
			url: base_url+'customizeorders/AjaxSingleView',
			type: 'POST',
			dataType: 'json',
			data: {id: data_id},
			success:function(data){
	...
	$(document).on('click','.dealer_custom_order_view_trigger',function(){
		var data_id=$(this).attr('data-id');
		$.ajax({
			url: tmp_base_url+'dealer-admin/view/orders',
			type: 'POST',
			dataType: 'json',
			data: {id: data_id},
			success:function(data){
	...
	var viewOrderTable=$('#viewOrderTable').DataTable({
	    "responsive": true,
	    "autoWidth": false,
	    "processing": true,
	    "pageLength": 10,
		...
		"order": [[ 1, "desc" ]],
	    "ajax": {
	        "url": base_url+"orders/viewOrderList",
			"data": function(d){
				d.filter_date = $("#inputDate").val();
			}
	    },
	    "columns": [
	        { "data": "no"},
	...
	$("#submitFilter").click(function(){
		$('#viewOrderTable').dataTable().fnClearTable();
	})
	//end new order
    ...
    //new order 
    $(document).on('click', '.view-order-export-list', function () {
        $.ajax({
            url: base_url + 'orders/viewOrderExport',
            data: {id:1},
            type: 'POST',
            dataType: 'json',
            success: function (data) {
...
function CustomizeDatatableDraw(frmname,ajaxurl,columnslist)
{
		var base_url=tmp_base_url+'Admin/';
  if(frmname!="customizebasejar"){		
	var dataarr=[{ "data": "no","visible":false},
	        { "data": "id", }];
  }else{
	  var dataarr=[{ "data": "id" }];
  }
	for(var i in columnslist) {
		  var item = columnslist[i]; 	
	...
	console.log(dataarr); 
	window[frmname] =$('#'+frmname).DataTable({
	    "responsive": true,
	    "autoWidth": false,
	    "processing": true,
	    "pageLength": 10,
...
$(function(){
	$(document).on('click','.uncompleted_order', function(){
		$("#order_status_id").val('');
		let id = $(this).data('id');
 ...
 function getDataFeature( cat_id ) {
