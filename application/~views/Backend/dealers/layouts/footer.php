...
<script src="<?= LAYOUT_URL; ?>plugins/summernote/summernote.js"></script>
<!-- Bootstrap WYSIHTML5 -->
...
<?php
/*customizedatatable call */
echo $drawCustomizeDataTable;
?>


<script>

$('.logout_dealer_trigger').click(function(){
			swal({
			...
			.then((Logout) => {
			  if (Logout) {
			    swal("Your Session will be Clear..", {
			      icon: "success",
			    }).then(() => {window.location.href = '<?= base_url() ?>dealer-admin/logout';});
			  } else {
...
$(document).ready(function(){
    ...
    $('.daterangepicker2').daterangepicker(
      ...
      function (start, end) {
        $('.daterangepicker2').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'))
      }
    )
