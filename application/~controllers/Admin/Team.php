...
class Team extends CI_Controller {
	function __construct() {
	 ...
	 public function index() {
	    if(hasPermission('team_index') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
	    }
		$data['DataResult']=$this->FunctionModel->Select('vidiem_team');
		$this->load->view('Backend/team-view',$data);
    }

	public function add() {
    	    if(hasPermission('team_add') != true){
    			$this->session->set_flashdata('class', "alert-danger");
    			$this->session->set_flashdata('icon', "fa-warning");
    			$this->session->set_flashdata('msg', "Access denied.");
    			redirect('Admin/dashboard', 'refresh');
			...
			$this->form_validation->set_rules('order_no','Order No.','required|numeric|greater_than[0]|max_length[3]');
			 if (!$this->form_validation->run() == FALSE) {
			   $InsertData=array(
		         ...
		         $result=$this->FunctionModel->Insert($InsertData,'vidiem_team');
				if($result >= 1){
		   ...
		   $this->load->view('Backend/team-add');
    }
	...
	public function edit($id = NULL) {
	    if(hasPermission('team_update') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
		...
		$this->form_validation->set_rules('order_no','Order No.','required|numeric|greater_than[0]|max_length[3]');
		  ...
		  $result=$this->FunctionModel->Update($UpdateData,'vidiem_team',array('id'=>$edit_id));
			...
			if ($result == 1) :
				...
				$this->session->set_flashdata('icon', "fa-warning");
				$this->session->set_flashdata('msg', "Something Went Wrong.");
          		redirect('Admin/team/edit/'.$edit_id, 'refresh');
        	endif;
			}
			$data['Edit_Result']=$this->FunctionModel->Select_Row('vidiem_team',array('id'=>$edit_id));
	...
	function edit_image_upload(){
	  if($_FILES['image']['size'] != 0){
		...
		$file_name=$this->FunctionModel->Select_Field('image','vidiem_team',array('id'=>$id));
	...
	public function status($id = NULL,$status = NULL) {
		...
		$result = $this->FunctionModel->Update($UpdateData,'vidiem_team',array('id'=>$id));
		if ($result == 1) :
        	$this->session->set_flashdata('class', "alert-success");
			$this->session->set_flashdata('icon', "fa-check");
	...
	public function delete($id = NULL) {
        ...
        $result = $this->FunctionModel->Delete('vidiem_team',array('id'=>$id));
		if ($result == 1) :
        	$this->session->set_flashdata('class', "alert-success");
			$this->session->set_flashdata('icon', "fa-check");
