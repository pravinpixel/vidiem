...
class ProjectModel extends CI_Model {
 ...
 public function Pagesevents(){
     ...
     public function complaintCode(){
       /* $code=$this->FunctionModel->Select_Field('code','vidiem_complaint_registration',array('code !='=>'WC-1001'),'id','DESC',1);
        if(empty($code)){
           return 'WC-1001';
        }else{
        ...
        $rowcnt=$this->FunctionModel->Row_Count('vidiem_complaint_registration',array());
        $rowcnt+=1;
        $code='WC-'.date("Y")."-".str_pad($rowcnt,4,"0",STR_PAD_LEFT);
    ...
    public function RegistrationCode(){
        $rowcnt=$this->FunctionModel->Row_Count('vidiem_product_registration',array());
        $rowcnt+=1;
        $code='REG-'.date("Y")."-".str_pad($rowcnt,4,"0",STR_PAD_LEFT);
        // print_r($this->db->last_query());
       //    die();
        return $code;
        /*if(empty($code)){
     ...
     public function getCities(){
         //$data=array('Andhra Pradesh''','''Arunachal Pradesh'',''Assam','Bihar','Chhattisgarh','Goa','Gujarat','Haryana','Himachal Pradesh','Jharkhand','Karnataka','Kerala','Madhya Pradesh','Maharashtra','Manipur','Meghalaya','Mizoram','Nagaland','Odisha','Punjab','Rajasthan','Sikkim','Tamil Nadu','Telangana','Tripura','Uttarakhand','Uttar Pradesh','West Bengal','Andaman and Nicobar Islands','Chandigarh','Dadra and Nagar Haveli','Daman and Diu','Delhi','Jammu and Kashmir','Ladakh','Lakshadweep','Puducherry','United Kingdom','United State of America (USA)','Canada','Singapore','Mauritius','Europe');
        $data=array('Andhra Pradesh','Arunachal Pradesh','Assam','Bihar','Chhattisgarh','Delhi','Goa','Gujarat','Haryana','Himachal Pradesh','Jharkhand','Karnataka','Kerala','Madhya Pradesh','Maharashtra','Manipur','Meghalaya','Mizoram','Nagaland','Odisha','Punjab','Rajasthan','Sikkim','Tamil Nadu','Telangana','Tripura Agartala','Uttar Pradesh','Uttarakhand','West Bengal','United Kingdom','United State of America (USA)','Canada','Singapore','Mauritius','Europe');
        return $data;
    }
 ...
 public function ClientMailCheck($email){
	...
	if($order['id']=='' || empty($order['id']))
	{
		$order=$this->FunctionModel->Select_Row('vidiem_customorder',array('inv_code'=>$code));
    ...
    public function ProductFilter($cat_id,$price,$filters,$type=NULL,$sub_cat=NULL,$sort,$stock,$sub_filters=null){
       ...
       // $this->db->select('p.id,p.slug,p.name,p.image,p.price,p.old_price,p.outofstock,p.list_description');
        ...
        if($type==2){
            $this->db->where('p.sub_cat_id',$sub_cat);
        }
    ...
    public function InvoiceCode(){
     ...
     public function InvoiceCode_test(){
        $code=$this->FunctionModel->Select_Field('inv_code','vidiem_order_26_07',array(),'inv_code','DESC',1);
        if(empty($code)){ return '0001'; }
        else{
             ++$code;
            return str_pad($code,'4','0',STR_PAD_LEFT);
...
Oggiam Thoraipakkam,<br>Chennai - 600097, Tamilnadu, INDIA.</li>
            ...
            <ul style="width:100%; display:inline-block; margin:0; padding:0;list-style:none;">
                ...
                if(!empty($order_data['billing_company_name'])){
                 ...
                 $invoice.='</li>
                <li style="font-size:14px;"><span style="width:22%;list-style:none;line-height:28px; display:inline-block;">Address</span> : &nbsp; '.@$order_data['billing_address'].' <br/> '.@$order_data['billing_address'].'
                </li>
                <li style="font-size:14px;"><span style="width:22%;list-style:none;line-height:28px; display:inline-block;">City-Zip</span> : &nbsp; '.@$order_data['billing_city'].'-'.$order_data['billing_zip'].'
                </li>
                <li style="font-size:14px;"><span style="width:22%;list-style:none;line-height:28px; display:inline-block;">State</span> : &nbsp; '.@$order_data['billing_state'].'
   ...
   $this->m_pdf->pdf->AddPage('P', // L - landscape, P - portrait
    ...
    public function Select_Orders(){
        $this->db->select('o.*,if(o.client_id=0,o.delivery_name,c.name) as name ,if(o.client_id=0,o.delivery_emailid,c.email) as email,if(o.client_id=0,o.delivery_mobile_no,c.mobile_no) as mobile_no');
        $this->db->join('vidiem_clients c','c.id=o.client_id', 'left');
        $this->db->order_by('o.inv_code','DESC');
    ...
    // new orders
    public function SelectOrderProductList($list= 'list',$where =array(),$custom =null,$search =[],$order =false)
    {
        /*$this->db->select("o.*,cat.name as category_name, 
        IF(inv_date IS NULL,o.created, inv_date) as invoice_date,
        op.price as price,
        op.qty as qty,
        if(o.client_id=0,o.delivery_name,c.name) as name ,
        if(o.client_id=0,o.delivery_emailid,c.email) as email,
        if(o.client_id=0 or c.mobile_no ='',o.delivery_mobile_no,c.mobile_no) as mobile_no,
        o.delivery_address as address,
        o.delivery_city as city,
        o.delivery_state as state,
        o.delivery_zip as zip_code,
        ...
        o.delivery_emailid as email,
        o.delivery_mobile_no as mobile_no,
        o.delivery_address as address,
        o.delivery_city as city,
        o.delivery_state as state,
        o.delivery_zip as zip_code,
        ...
        $this->db->join('vidiem_order o','op.order_id=o.id','left');
        $this->db->join('vidiem_products p','op.product_id=p.id','left');
        $this->db->join('vidiem_category cat','p.cat_id=cat.id','left');
        $this->db->join('vidiem_clients c','c.id=o.client_id', 'left');
        $this->db->where(array('o.status !='=>4,'o.payment_status'=>'success'));
        $this->db->where($where);
        ...
        if($list=='list'){  
            return $this->db->get_where('vidiem_order_products op',array(),$this->input->post('length'),(int)$this->input->post('start'))->result_array();   
        }else{  
            $count=$this->db->count_all_results('vidiem_order_products op'); 
            return (int)$count; 
        } 
    }
    ...
    {
       /* $this->db->select("
        o.inv_code as `E-shop Invoice`,
        IF(inv_date IS NULL,o.created, inv_date) as `E-shop Invoice Date`,
        if(o.client_id=0,o.delivery_name,c.name) as `Customer Name` ,
        o.delivery_address as address,
        if(o.client_id=0 or c.mobile_no ='',o.delivery_mobile_no,c.mobile_no) as `Mobile No`,
        if(o.client_id=0,o.delivery_emailid,c.email) as Email,
        o.delivery_city as city,
        o.delivery_state as state,
        o.delivery_zip as zip_code,
        ...
        $this->db->select("  o.inv_code as `E-shop Invoice`,
        IF(inv_date IS NULL,o.created, inv_date) as `E-shop Invoice Date`,
        o.delivery_name as `Customer Name` ,
        o.delivery_address as address,
        o.delivery_mobile_no as `Mobile No`,
        o.delivery_emailid as Email,
        o.delivery_city as city,
        o.delivery_state as state,
        o.delivery_zip as zip_code,
        ...
        op.qty as Quantity,
        p.name as `Product Name`,
        o.bank_ref_num as `Bank Reference No`
        "); 
        ...
        $this->db->join('vidiem_clients c','c.id=o.client_id', 'left');
        $this->db->where(array('o.status !='=>4,'o.payment_status'=>'success'));
        return $this->db->get_where('vidiem_order_products op')->result_array();
    }
    ...
    public function Select_unCompletedOrders(){
	 ...
	 public function Select_unCompletedOrders_cron(){
        $this->db->select('o.*,if(o.client_id=0,o.delivery_name,c.name) as name ,if(o.client_id=0,o.delivery_emailid,c.email) as email,if(o.client_id=0,o.delivery_mobile_no,c.mobile_no) as mobile_no');
        $this->db->join('vidiem_clients c','c.id=o.client_id', 'left');
		if(count($where)>0){
		$this->db->where($where); 
         ...
         $this->db->order_by('o.id','DESC');
        $query=$this->db->get_where('vidiem_order o',array('o.status !='=>4,'o.payment_status !='=>'success',"o.created>="=>date("Y-m-d H:i:00",strtotime("-30 minutes")),"o.created<="=>date("Y-m-d H:i:00",strtotime("-15 minutes"))));
		//$query=$this->db->get_where('vidiem_order o',array('o.status !='=>4,'o.payment_status !='=>'success',"o.created>="=>date("Y-m-d H:i:00",strtotime("2022-02-11 12:10:04")),"o.created<="=>date("Y-m-d H:i:00",strtotime("2022-02-11 12:30:04"))));
    ...
    public function ProductSearch($search_term){
        ...
        $query=$this->db->get_where('vidiem_products p',array('p.product_id'=>0, 'p.exclusive'=> 0));
        return $query->result_array();
    }
//recipe
    ...
    public function ProuctListing(){
        $this->db->select('p.*,c.name as category,s.name as sub_category');
        $this->db->join('vidiem_category s','s.id=p.sub_cat_id','left');
        $this->db->join('vidiem_category c','c.id=p.cat_id','left');
    ...
    public function NewOrderNotification($order_id){
        $order_info=$this->FunctionModel->Select_Fields_Row('client_id,inv_code,amount,billing_emailid,billing_mobile_no','vidiem_order',array('id'=>$order_id));
...
<table class="m_-6053241845862020151m_1333870210502687750MsoNormalTable" border="0" cellpadding="0" width="100%" style="width:100.0%"><tbody><tr><td width="10" style="width:7.5pt;padding:5.25pt 0cm 5.25pt 0cm"><p class="MsoNormal">&nbsp;<u></u><u></u></p></td><td style="padding:5.25pt 0cm 5.25pt 0cm"><div style="border:none;border-bottom:solid #d6d4d4 1.0pt;padding:0cm 0cm 8.0pt 0cm"><p style="margin-right:0cm;margin-bottom:5.25pt;margin-left:0cm;border:none;padding:0cm"><span style="font-size:13.5pt;font-family:&quot;Arial&quot;,&quot;sans-serif&quot;;color:#555454;text-transform:uppercase">Important Security Tips:<u></u><u></u></span></p></div><ol start="1" type="1"><li class="MsoNormal" style="color:#555454"><span style="font-size:10.0pt;font-family:&quot;Arial&quot;,&quot;sans-serif&quot;">Always keep your account details safe.<u></u><u></u></span></li><li class="MsoNormal" style="color:#555454"><span style="font-size:10.0pt;font-family:&quot;Arial&quot;,&quot;sans-serif&quot;">Never disclose your login details to anyone.<u></u><u></u></span></li><li class="MsoNormal" style="color:#555454"><span style="font-size:10.0pt;font-family:&quot;Arial&quot;,&quot;sans-serif&quot;">Change your password regularly.<u></u><u></u></span></li><li class="MsoNormal" style="color:#555454"><span style="font-size:10.0pt;font-family:&quot;Arial&quot;,&quot;sans-serif&quot;">Should you suspect someone is using your account illegally, please notify us immediately.<u></u><u></u></span></li></ol></td><td width="10" style="width:7.5pt;padding:5.25pt 0cm 5.25pt 0cm"><p class="MsoNormal">&nbsp;<u></u><u></u></p></td></tr></tbody></table>
    ...
    public function productRegistrationExport(){
        $this->db->select('code as `Ref Code`,Product as `Product`,jdate as `Purchase Date`,serialnumer as `Serial Number`,dealername as `Dealer Name`,gender as `Gender`,name as `Name`,age as `Age`,email as `Email`,mobile as `Mobile No`,occupation as `Occupation`,address as `Address`,city as `City`,state as `State`, country as Country, pincode as `Pincode`,created as `Created`');
    ...
    public function getProductRegistrationById($id) {
        return $this->db->select('*')
                ->where('id', $id)
                ->get('vidiem_product_registration')
                ->row();
    }

    public function getComplainRegistrationById($id) {
        return $this->db->select('*')
                        ->where('id', $id)
                        ->get('vidiem_complaint_registration')
                        ->row();
    }

   public function registrationCategoryList($list='list',$where=array(),$custom=null,$search=[],$order=false){
        $this->db->select('*');
        $this->db->where($where);   
        if(!empty($search)){    
            foreach($search as $filed=>$value){ 
        ...
        if($list=='list'){  
            return $this->db->get_where('vidiem_registration_categories',array(),$this->input->post('length'),(int)$this->input->post('start'))->result_array();   
        }else{  
            $count=$this->db->count_all_results('vidiem_registration_categories'); 
            return (int)$count; 
        }   
    }

    public function registrationProductList($list='list',$where=array(),$custom=null,$search=[],$order=false)
    {
        $this->db->select('p.*,c.category_name as category_name');
        $this->db->join('vidiem_registration_categories c','c.id=p.category_id');
        $this->db->where($where);   
        if(!empty($search)){    
            foreach($search as $filed=>$value){ 
                $this->db->or_like($filed,$value);      
        ...
        if($list=='list'){  
            return $this->db->get_where('vidiem_registration_products p',array(),$this->input->post('length'),(int)$this->input->post('start'))->result_array();   
        }else{  
            $count=$this->db->count_all_results('vidiem_registration_products p'); 
            return (int)$count; 
        }   
    }
    public function getRegistrationProductByCategoryId($id)
    {
        return $this->db->select('*')
                ->where('category_id', $id)
                ->get('vidiem_registration_products')
                ->row();
    }

    public function getRegistrationCategoryById($id)
    {
        return $this->db->select('*')
                ->where('id', $id)
                ->get('vidiem_registration_categories')
                ->row();
    }

    public function getRegistrationProductById($id)
    {
        return $this->db->select('*')
                ->where('id', $id)
                ->get('vidiem_registration_products')
