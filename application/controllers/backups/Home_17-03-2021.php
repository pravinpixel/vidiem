...
class Home extends CI_Controller {
    ...
    public function event($id=NULL){
     ...
     public function checkout(){
        ...
        if(empty($data['same'])){$data['same']=1;}
    ...
    public function payumoney_failure(){
     ...
     public function contact_us(){
      ...
      // $this->sendCovidMail($from_mail);
         $this->FunctionModel->sendmail($from_mail, $msg2, 'Reference Number','care@vidiem.in');
        	    $this->session->set_flashdata('title', "Success");   
                $this->session->set_flashdata('msg', "Email Send Successfully.");     
                $this->session->set_flashdata('type', "success");
...
public function recipe_search($start=0){
    ...
    public function password_check(){
     ...
     public function register(){
        ...
        $this->form_validation->set_rules('mobile_no','Mobile No.','required|is_unique[vidiem_clients.mobile_no]');
    ...
    public function Recipe_Videos(){
     ...
     public function press_release(){
      ...
      public function product_registration(){
        $data['menu_id']=4;
               $data['category']=$this->FunctionModel->Select('vidiem_category',array('status'=>1));
     ...
     public function complaint_registration(){
        ...
        $this->form_validation->set_rules('dealername','Dealer Name','required');
         $this->form_validation->set_rules('remarks','Complaint Remarks','required');
        $this->form_validation->set_rules('name','Name','required');
        $this->form_validation->set_rules('email','Email','required|valid_email');
        $this->form_validation->set_rules('mobile','Mobile Numer','required');
      //  $this->form_validation->set_rules('mobile2','Alternative Mobile Numer','required');
        ...
        if ($this->form_validation->run() === TRUE) {
           ...
           $msg2='<style>table{background-color:#e6e6e6;}tr > td{padding:10px; font-size: 18px;} .tc{text-align:center;}</style>
            ...
            $this->FunctionModel->sendmail($from_mail, $msg2, 'Complaint Code','care@vidiem.in');
              // $this->sendCovidMail($from_mail);
                $this->session->set_flashdata('title', "Success");   
                $this->session->set_flashdata('msg', "Email Send Successfully.");     
                $this->session->set_flashdata('type', "success");
...
public function AjaxOrderCancel(){
  ...
  //$this->ion_auth->register($username, $password, $email, $additional_data, $group)
	...
	$addressformhtml='<h4 class="text-dark mb-3 shippingaddressdivupdate" data-aos="fade-up" data-aos-delay="50" data-aos-duration="1000">Shipping Address Update</h4>
					<div class="bg-white mb-5 p-4 shippingaddressdivupdate" data-aos="fade-up" data-aos-delay="50" data-aos-duration="1000">';
					...
					$addressformhtml .='<form method="POST" action="" id="shippingaddressFormupdate">
						...
						<div class="row">
							...
							<div class="col-sm-12 col-md-6">
								<div class="md-form">
								  <input type="text" id="mobile_no_ship" class="form-control jsrequired" name="mobile_no" value="'.$Edit_Info['mobile_no'].'">
...
Oggiam Thoraipakkam,<br>Chennai - 600097, Tamilnadu, INDIA.</li>
   ...
   // $this->load->view('Backend/pdf-page',$data);
	...
	{
	  $this->FunctionModel->sendmail_smtp("", "", "", "");   
