  ...
  public function vidiem_for_you(){
	...
	if(!empty($id) && $id!=""){
		...
		$basemotor.='<div class="col-12 col-sm-8 col-md-8 col-lg-9"><div class="motor-select-scroll">';
		 if(count($basemotorlist)>0){
						...
						if(	$base_col['price']!=0)
						{
							$basemotor.='<span>'.$base_col['price'].'</span>'; 
						}		
		...
		$basejar='  <div class="jar-scroll">';
		 if(count($jarlist)>0){
			...
			$basejar.='    <div>
                              ...
                              <p class="jar-options-selected"><strong>1 Jar Selected</strong><p>
                              </div>
                           </div> </div> ';	
    ...
    public function calculatecartprice($cartid)
	{
		 $cartarr = $this->CustomizeModel->getDataActiveById($cartid,"vidiem_carts","cart_id");					
				if($cartarr->cart_id)
				{
					 $this->db->select(" (if(b.base_id is not null, b.price,'0')
											+
											 if( bc.bm_color_id is not null, bc.price,'0')
											+
											if( m.motor_id is not null, m.price,'0')
											+
											 if( p.package_id is not null, p.price,'0')) as price");
					...
					return number_format($dataarr[0]['price']+$jarpricearr[0]['price']);
				}
