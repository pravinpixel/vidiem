		...
		$basecolor="<div class='body-color'>";
		 if(count($basecolorlist)>0){
				...
				$basecolor.='</div>  <div class="next-btn">
                           ...
                           <button disabled type="button" class="red-btn" id="body-color-next">Next &nbsp; <i class="lni lni-arrow-right"></i></button>
                        </div>
		 ...
		 if(count($basemotorlist)>0){
				...
				$basemotor.='</div> </div> 
                        <div class="col-12"> <div class="next-btn">
                           ...
                           <button disabled type="button" class="red-btn"   href="#" id="motor-next">Next &nbsp; <i class="lni lni-arrow-right"></i></button>
                        </div></div>
		 ...
		 if(count($jarlist)>0){
						...
						$basejar.='
                              ...
                              <div class="select-jar selectedjar-'.$jar['jar_id'].'">
                                 <a href="'.base_url("uploads/customizeimg/jar/".$jar['basepath']).'" data-fancybox data-caption="Jar1">
    ...
    public function getselecteditems($cartid)
	{
		...
		$strselecteditem="";	
				if($cartarr->cart_id)
				{
					...
					{
					$strselecteditem.='<div class="selected-body">Body: <strong>'.$dataarr[0]['basetitle'].'</strong><span class="selected-body-price">Rs.'.number_format($dataarr[0]['baseprice']).'</span></div>';
					}
					
					if(!empty($dataarr[0]['bm_color_id']))
					{
					$strselecteditem.='   <div class="selected-color">Color: <strong>'.$dataarr[0]['title'].'</strong><span class="selected-color-price">Rs.'.number_format($dataarr[0]['colorprice']).'</span></div>';
					}
					...
					{
					$strselecteditem.='   <div class="selected-color">Motor: <strong>'.$dataarr[0]['motorname'].'</strong><span class="selected-color-price">Rs.'.number_format($dataarr[0]['motorprice']).'</span></div>';
					}
					
					if(!empty($dataarr[0]['canvas_text']) && $dataarr[0]['canvas_text']!='')
					{
					$strselecteditem.='   <div class="selected-color">Imported Text: <strong>'.$dataarr[0]['canvas_text'].'</strong><span class="selected-color-price">Rs.'.number_format($dataarr[0]['canvas_price']).'</span></div>';
					}
					
					if(!empty($dataarr[0]['package_id']) && $dataarr[0]['package_id']!='')
					{
					$strselecteditem.='   <div class="selected-color">Package: <strong>'.$dataarr[0]['packagename'].'</strong><span class="selected-color-price">Rs.'.number_format($dataarr[0]['package_price']).'</span></div>';
					}
...
Oggiam Thoraipakkam,<br>Chennai - 600097, Tamilnadu, INDIA.</li>
   ...
   // $this->load->view('Backend/pdf-page',$data);
	...
	$urlslug= base_url('product/'.$listdata['slug']); 
	 ...
	 public function service_centers_new(){
        $data['menu_id']=4;
