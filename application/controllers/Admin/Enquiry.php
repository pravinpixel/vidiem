...
class Enquiry extends CI_Controller {
	function __construct() {
	 ...
	 public function index() {
	    if(hasPermission('enquiry_index') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
		}
		$data['DataResult']=$this->FunctionModel->Select_Fields('id,type,name,email,mobile,city,ref_no,enquiry,message,reply_msg,created','vidiem_enquiry',array('status !='=>0,'type'=>1));
		$this->load->view('Backend/enquiry-view',$data);
    }

	public function delete($id = NULL) {
	    if(hasPermission('enquiry_delete') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
    ...
    public function ReplyEmail(){
    	...
    	$subject='Reply Mail From Stock Phonenix';
