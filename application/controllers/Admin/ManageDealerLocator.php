...
class ManageDealerLocator extends CI_Controller {
    ...
    public function index(){
		if(hasPermission('dealer_locator_index') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
		} 
        $this->load->view('Backend/ManageDealerLocator/index');
    }

    public function create()
    {
        if(hasPermission('dealer_locator_add') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
        ...
        $this->load->view('Backend/ManageDealerLocator/form',$data);
    }
    ...
    public function save()
    {
        ...
        if($this->input->method() == 'post') {
            ...
            if ($this->form_validation->run() == FALSE) { 
                ...
                return $this->load->view('Backend/ManageDealerLocator/form',$data); 
            }
            ...
            $result = $this->FunctionModel->Insert($createdatavalue,'vidiem_dealerlocator',array('center_id' => $id));
            ...
            return redirect('Admin/ManageDealerLocator/index');
            }
    ...
    {
        if(hasPermission('dealer_locator_update') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
		 ...
		 $data['action'] =  "Edit";
        $data['dataitems'] = $this->CustomizeModel->getDataArrById($id,"vidiem_dealerlocator","center_id");
    ...
    public function update($id)
    {
        ...
        if($this->input->method() == 'post') {
            ...
            $result = $this->FunctionModel->Update($updatedatavalue,'vidiem_dealerlocator',array('center_id' => $id));
            if( $result ) {
                $this->session->set_flashdata('class', "alert-success");
                $this->session->set_flashdata('icon', "fa-success");
    ...
    {
        if($this->input->method() == 'post') {
            $dataarr = $this->CustomizeModel->getDataArrById($id,"vidiem_dealerlocator","center_id");
            $update_status = [
            ...
            $result = $this->FunctionModel->Update($update_status,'vidiem_dealerlocator',array('center_id' => $id));
    ...
    public function delete($id = NULL) {
        ...
        if(hasPermission('dealer_locator_delete') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
        ...
        if(!isset($dataarr->center_id)){
            ...
            return $this->load->view('Backend/ManageDealerLocator/index');
        }
    ...
    public function display_datatble_list(){
        ...
        $DataResult=$this->SerivceModel->ManageDealerLocatorList('list',$where,$custom,$searchQuery,$order);
        $count=$this->SerivceModel->ManageDealerLocatorList('count',$where,$custom,$searchQuery);
        $data = array();
       ...
       if($DataResult){
        ...
        $data[]=array(
           ...
           'status'              => '<div calss="row" ><a href="#" data-message="Are you want to update status" data-href="' . base_url('Admin/ManageDealerLocator/status/'.$row['center_id']) . '" id="tooltip" data-method="POST" data-title="POST"  data-toggle="modal" data-target="#-status-"><span class="label '.$status_color.' status-span">'.$status.'</span></a>',
           'action'              => '<div calss="row"> <a href="'.base_url('Admin/ManageDealerLocator/edit/'.$row['center_id']).'" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span></a>
                                   <a href="#" data-message="Are you sure want to delete ? " data-href="' . base_url('Admin/ManageDealerLocator/delete/'.$row['center_id']) . '" id="tooltip" data-method="POST" data-title="POST"  data-toggle="modal" data-target="#-status-" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span></a></div>'
        );
        if($order['type']=='desc' && $order['field']=='id' ){
            $no++;
        }else if($order['type']=='asc' && $order['field']=='id' ){
   ...
   function edit_image_upload($value, $id) {
	   ...
	   if($_FILES['basepath']['size'] != 0){
		...
		$file_name=$this->FunctionModel->Select_Field('basepath','vidiem_dealerlocator',array('center_id'=>$id));
