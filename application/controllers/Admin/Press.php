...
class Press extends CI_Controller {
	function __construct() {
	 ...
	 public function index() {
	    if(hasPermission('press_index') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
		} 
		$data['DataResult']=$this->FunctionModel->Select('vidiem_press');
		$this->load->view('Backend/press-view',$data);
    }

	public function add() {
    	    if(hasPermission('press_add') != true){
    			$this->session->set_flashdata('class', "alert-danger");
    			$this->session->set_flashdata('icon', "fa-warning");
    			$this->session->set_flashdata('msg', "Access denied.");
    			redirect('Admin/dashboard', 'refresh');
			...
			$this->form_validation->set_rules('pressdate','Date','required');
			...
			$this->form_validation->set_rules('order_no','Order No.','required|numeric|greater_than[0]|max_length[3]');
			 if (!$this->form_validation->run() == FALSE) {
			   $InsertData=array(
					...
					'pressdate'     => $this->input->post('pressdate'),
			...
			if(!empty($file_name)){$InsertData['file']=$file_name;}
		         $result=$this->FunctionModel->Insert($InsertData,'vidiem_press');
				if($result >= 1){
					$this->session->set_flashdata('class', "alert-success");
					$this->session->set_flashdata('icon', "fa-check");
					$this->session->set_flashdata('msg', "Press Added Successfully.");
					...
					redirect('Admin/press/add', 'refresh');
				}
			}
		   //$data['Pages']=$this->ProjectModel->Pages();
		   $this->load->view('Backend/press-add');
    }

	function image_upload(){
	  if($_FILES['image']['size'] != 0){
		$upload_dir = './uploads/press';
...
if($_FILES['file']['size'] != 0){
	...
	public function edit($id = NULL) {
	    if(hasPermission('press_update') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
		...
		$this->form_validation->set_rules('order_no','Order No.','required|numeric|greater_than[0]|max_length[3]');
		  ...
		  $result=$this->FunctionModel->Update($UpdateData,'vidiem_press',array('id'=>$edit_id));

			if ($result == 1) :
				$this->session->set_flashdata('class', "alert-success");
				$this->session->set_flashdata('icon', "fa-check");
				$this->session->set_flashdata('msg', "Press Updated Successfully.");
				redirect('Admin/press','refresh');
			else :
          		$this->session->set_flashdata('class', "alert-danger");
				$this->session->set_flashdata('icon', "fa-warning");
				$this->session->set_flashdata('msg', "Something Went Wrong.");
          		redirect('Admin/press/edit/'.$edit_id, 'refresh');
        	endif;
			}
			$data['Edit_Result']=$this->FunctionModel->Select_Row('vidiem_press',array('id'=>$edit_id));
		   $this->load->view('Backend/press-edit',$data);
    }
	...
	function edit_image_upload(){
	  if($_FILES['image']['size'] != 0){
		...
		$file_name=$this->FunctionModel->Select_Field('image','vidiem_press',array('id'=>$id));
...
if($_FILES['file']['size'] != 0){
	...
	public function status($id = NULL,$status = NULL) {
		...
		$result = $this->FunctionModel->Update($UpdateData,'vidiem_press',array('id'=>$id));
		if ($result == 1) :
        	$this->session->set_flashdata('class', "alert-success");
			$this->session->set_flashdata('icon', "fa-check");
	...
	public function delete($id = NULL) {
	    if(hasPermission('press_delete') != true){
			$this->session->set_flashdata('class', "alert-danger");
			$this->session->set_flashdata('icon', "fa-warning");
			$this->session->set_flashdata('msg', "Access denied.");
			redirect('Admin/dashboard', 'refresh');
        ...
        $result = $this->FunctionModel->Delete('vidiem_press',array('id'=>$id));
		if ($result == 1) :
        	$this->session->set_flashdata('class', "alert-success");
			$this->session->set_flashdata('icon', "fa-check");
