...
class customizemotor extends CI_Controller {
    ...
    public function index(){
        ...
        $this->load->view('Backend/customizemotor/index');
    }
    ...
    public function create()
    {
        if(hasPermission('customizable_add') != true){
		 ...
		 $data['action'] =  "Create";
        $this->load->view('Backend/customizemotor/form',$data);
    }
    ...
    public function save()
    {
        ...
        if($this->input->method() == 'post') {
            ...
            if ($this->form_validation->run() == FALSE) { 
				  $data['action']="Create";
				  $data['dataitems']['motorname'] =  ltrim($this->input->post('motorname'));	
                return $this->load->view('Backend/customizemotor/form',$data); 
            }
			...
			//die();
			 ...
			 $createdatavalue = [
                "motorname"    => ltrim($this->input->post('motorname')),
            ...
            return redirect('Admin/customizemotor/index');
            }
    ...
    public function update($id)
    {
        ...
        if($this->input->method() == 'post') {
            ...
            $result = $this->FunctionModel->Update($updatedatavalue,'vidiem_motors',array('motor_id' => $id));
            if( $result ) {
                $this->session->set_flashdata('class', "alert-success");
                $this->session->set_flashdata('icon', "fa-success");
    ...
    {
        if($this->input->method() == 'post') {
            $dataarr = $this->CustomizeModel->getDataArrById($id,"vidiem_motors","motor_id");
            $update_status = [
            ...
            $result = $this->FunctionModel->Update($update_status,'vidiem_motors',array('motor_id' => $id));
    ...
    public function display_datatble_list(){
        ...
        $DataResult=$this->CustomizeModel->customizemotorList('list',$where,$custom,$searchQuery,$order);
        $count=$this->CustomizeModel->customizemotorList('count',$where,$custom,$searchQuery);
        $data = array();
       ...
       if($DataResult){
        ...
        $data[]=array(
		   ...
		   'price'           => $row['price'],		     
           'status'              => '<div calss="row" ><a href="#" data-message="Are you want to update status" data-href="' . base_url('Admin/customizemotor/status/'.$row['motor_id']) . '" id="tooltip" data-method="POST" data-title="POST"  data-toggle="modal" data-target="#-status-"><span class="label '.$status_color.' status-span">'.$status.'</span></a>',
           'action'              => '<div calss="row"> <a href="'.base_url('Admin/customizemotor/edit/'.$row['motor_id']).'" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span></a>
                                   <a href="#" data-message="Are you sure want to delete ? " data-href="' . base_url('Admin/customizemotor/delete/'.$row['motor_id']) . '" id="tooltip" data-method="POST" data-title="POST"  data-toggle="modal" data-target="#-status-" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span></a></div>'
        );
        if($order['type']=='desc' && $order['field']=='id' ){
            $no++;
        }else if($order['type']=='asc' && $order['field']=='id' ){
   ...
   function edit_image_upload($value, $id) {
	   ...
	   if($_FILES['basepath']['size'] != 0){
		...
		$file_name=$this->FunctionModel->Select_Field('basepath','vidiem_motors',array('motor_id'=>$id));
