	   ...
	   foreach($finalorder as $key=>$orderdata)
	   {
        ...
        $order_id=$dataarr['order_id'];
         ...
         if(empty($code)){
			...
			if($mobile!=''){
			  SMS($mobile,"Thank you for shopping with us. Your order number ".$inv_code." is under process. We'll share the tracking details once the shipment is ready.",'1107164362643761375'); 
			 }
...
public function recipe_search($start=0){
    ...
    /*    public function product_registration(){
     ...
     public function product_registration(){
        ...
        $this->form_validation->set_rules('product','Product','required');
        $this->form_validation->set_rules('serialnumer','Serial number','required|is_unique[vidiem_product_registration.serialnumer]|regex_match[/^[Cc][Hh][A-Za-z0-9]{13}$/]');
  ...
  public function vidiem_for_you(){
        ...
        $searchQuery = [];
		 ...
		 $data['typeoflid']=$this->CustomizeModel->typeoflidList('list',$where,$custom,$searchQuery,'');
			...
			if($cart_id)
			{
				...
				$where=array("p.isactive = "=>"1",
							"p.base_id"=>	$data['cartitems']['bodyinfo'][0]['base_id']);	
				...
				$where=array("p.isactive = "=>"1",
							"p.base_id"=>	$data['cartitems']['bodyinfo'][0]['bm_color_id']);	
	...
	//	$alljarlist = $this->CustomizeModel->combinebasejarexclude('list',$where,'','',$order);
		...
		$basemotor='	 <div class="col-12 col-sm-4 col-md-4 col-lg-3 mob-motor-hide">
                                <p class="text-center">
                                    <img id="motorimg" src="'.base_url('uploads/customizeimg/basemotor/'.$basemotorlist[0]['basepath']).' " alt="" class="img-fluid"/>
                                </p>
                            </div>';		
						...
						if(	$motor['motorprice']!=0)
						{
                            ...
                            $basemotor.=' 
                               ...
                               <div class="d-none motor_view_info" id="motor_view_info_'.$motor['motor_id'].'">
                                <a href="#" class="link" onclick="callMotorModal('.$motor['motor_id'].')">view info</a>
                              </div>
                           </div>';
	...
	if(!empty($id) && $id!=""){
		...
		if(count($alljarlist)>0){
						   ...
						   foreach($alljarlist as $jar) {
							   ...
							   $description = (!empty($jar['description']) && $jar['description'] != '') ?  '<span style="text-align: end" class="fa fa-info-circle" title="'.$jar['description'].'"></span>' : "";
                               
							   
						$otherjar.='
							  
                              <div class="select-jar selectedjar-'.$jar['jar_id'].'">
                                 <a href="'.base_url("uploads/customizeimg/jar/".$jar['basepath']).'?t='.time().'" data-fancybox data-caption="'.$jar['description'].'">
                                    '.$description.'
                                     <img src="'.base_url("uploads/customizeimg/jar/".$jar['basepath']).'?t='.time().'" alt="" class="img-fluid"/>
                                 </a>
 ...
 public function removeotherjars(){
		...
		$otherjar='  <div class="model-jars">';
				...
				if(count($alljarlist)>0) {		
						...
						$otherjar.='
                              ...
                              <div class="select-jar selectedjar-'.$jar['jar_id'].'">
                                 <a href="'.base_url("uploads/customizeimg/jar/".$jar['basepath']).'?t='.time().'" data-fancybox data-caption="Jar1">
                                     <img src="'.base_url("uploads/customizeimg/jar/".$jar['basepath']).'?t='.time().'" alt="" class="img-fluid"/>
                                 </a>
                                 <span class="jar-name">'.$jar['name'].'</span>
                                 <span>Rs. '.number_format($jar['price']).'</span>                                            
    ...
    public function getselecteditems($cartid)
	{
	   ...
	   public function customize_cart(){
		...
		$data['packageprice']=$this->CustomizeModel->calculatepackageprice($cart_id,$data['cartitems']['bodyinfo'][0]['base_id'],count($data['cartitems']['jarinfo']));
	...
	//echo	$previous_url = $this->session->userdata('previous_url');
	//print_r($this->session);
	//die();
		
			$data['packageprice']=$this->CustomizeModel->calculatepackageprice($cart_id,$cartitems['bodyinfo'][0]['base_id'],count($cartitems['jarinfo']));
	   ...
	   public function custompayment(){
        ...
        if(($shippingaddressid!='' && $shippingmethodid!='' && $confirmcart!='') || $client_id==''){
			...
			// Payment gateway parameters for Customisation products
			
			$packageprice=$this->CustomizeModel->calculatepackageprice($cart_id,$cartitems['bodyinfo'][0]['base_id'],count($cartitems['jarinfo']));
				...
				// product customisation parice for Rs. 1 order testing
				$amount=1;
...
$invoice='
   ...
   <div class="container inCon">
        <div style="float:left;"><h1 style="color:#00BFFF;"><img src="'.base_url('assets/front-end/images/logo.png').'" style="display:block; margin:4px auto 0 auto"/></h1></div>
        <div style="width:30%;float:right;"><h1 style="color:#000000;">PERFORMA INVOICE</h1></div>
        <p style="clear:both;"></p>
    <div class="header_bottom" style="width:100%; padding:10px 0;">
        <div class="detail" style="float:left; width:35%; margin-top:-15px;">
            <ul style="width:100%; display:inline-block; margin:0; padding:0;list-style:none;">
...
Oggiam Thoraipakkam,<br>Chennai - 600097, Tamilnadu, INDIA.</li>
   ...
   // $this->load->view('Backend/pdf-page',$data);
    ...
    {
        $this->load->view('vidiem_customize_start');
    }

    public function getMotorById()
    {
        $id = $this->input->post('id');
        $data['motor'] = $this->FunctionModel->Select_Row('vidiem_motors',array('motor_id'=> $id));
